...
write_ui_env() {
	info "UI runtime versions (ui/.env)"
	local ui_version api_version
	ui_version="$(node -p "require('./ui/package.json').version")"
	api_version="$(grep -m1 '^version = ' api/pyproject.toml | sed 's/.*"\(.*\)".*/\1/')"
	cat > ui/.env <<EOF
# Generated by scripts/setup.sh — server-only (\$env/dynamic/private)
UI_VERSION=${ui_version}
API_VERSION=${api_version}
EOF
	ok "ui/.env → UI_VERSION=${ui_version}, API_VERSION=${api_version}"
}
