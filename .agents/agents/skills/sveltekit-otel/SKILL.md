---
name: sveltekit-otel
description: Design, implement, audit, and verify SvelteKit-native OpenTelemetry tracing across browser, server, and downstream APIs.
---
...
- distinguishable across browser, SvelteKit server, and downstream services without relying on accidental backend presentation.
...
initial request
  ...
  -> downstream API/database/cache/queue
```
...
4. Manual instrumentation for domain semantics and missing lifecycle milestones.
...
### 9. Propagate to downstream services

Prefer automatic propagation from supported HTTP clients. Verify actual headers and resulting parentage.

For custom transports, queues, WebSockets, or jobs:

- inject/extract context using the OTel propagator;
- use links when processing is asynchronous or fan-out breaks strict parent-child timing;
- document retry semantics;
- avoid reusing expired context as an active parent indefinitely.
...
Test log correlation from inside an active SvelteKit load/action and inside a downstream-client span.
...
- queues, jobs, and asynchronous continuations;
